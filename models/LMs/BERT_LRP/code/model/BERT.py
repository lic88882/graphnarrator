...
from models.LMs.BERT_LRP.code.util.lrp import *
...
class BERTEncoder(nn.Module):
    def __init__(self, config):
        ...
        # self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(config.num_hidden_layers)])  # Why deepcopy multiple identical layers?
...
class MyBertModel(nn.Module):
    ...
    def backward_lrp(self, relevance_score):
        ...
        relevance_score = self.embeddings.backward_lrp(relevance_score)  # This is just an entity mapping
        return relevance_score

    def backward_gradient(self, sensitivity_grads):
        classifier_out = func_activations['model.bert.pooler']
        embedding_output = func_activations['model.bert.embeddings']
